...
categories
```
To speed things up, we build a couple functions to extract a tidy tier-specific dataset and to plot it.
```{r}
get_tier <- function(dataset, tier_no, results_no){
...
plot_tier <- function(dataset, tier_no, results_no){
  col_name = sprintf("Tier%d", tier_no)
  p <-
    ggplot(get_tier(dataset, tier_no, results_no), aes(x = N_items, y = reorder(!!sym(col_name), N_items))) + # col_name is a string, must be a symbol
    geom_col() +
...
```{r}
p1 <- plot_tier(categories, 1, 15)
p2 <- plot_tier(categories, 2, 15) 
p3 <- plot_tier(categories, 3, 15)
p4 <- plot_tier(categories, 4, 15) 
grid.arrange(p1, p2, p3, p4, ncol = 2)
```
By means of these simple grid of plots we see some important characteristics of the market: the drugs dominates all the other product tyoes, and among them cannabis and weed are by far the most sold products. Moreover, we notice that there are only a few items having up to four tiers in the category feature. We may hypotize it is so either because the products are even more specific than others, or due to a wrong classification.
```{r}
#eliminate tier 4 by incorporating them in tier 3 category?
```
