...
Upon conversion, PriceBTC is represented with the scientific notation due to its many decimal digits. We round it to 4 decimals, which is precise enough.
```{r}
agora <-
  agora %>%
...
agora <-
  ...
  filter(!is.na(PriceBTC))
```
Bitcoin are a volatile currency, and they may be less informative than a standard currency: a feature expressing the value in euros or dollars may be useful. We recall that this dataset dates back to 2014/2015, so we need to look up the average value of bitcoin in these years.
```{r}
#https://freecurrencyrates.com/en/exchange-rate-history/BTC-EUR/2015 shows min and max per year, take the mean between max and min of two years
btc_eur_rate = 302.495
agora <-
  agora %>%
  mutate(PriceEUR = round(PriceBTC * btc_eur_rate, 2), after = PriceBTC) #check how to add column near PRICEBTC
agora
```
