...
```
We see that the top four vendors have a similar number of items listed, while from the fifth on there is a noticeable gap. Probably the elite vendors have the largest sales volume and dominate the market, at the expenses of the other, less renowned vendors.
...
top_20_vendors %>%
  ...
  geom_treemap_text()
```
Another indicator of a vendor's weight on the market could be the *Rating* feature.
```{r}
agora %>%
  ...
  arrange(desc(Rating))
```
First thing we notice are two misparsed row, which we remove. Secondly, we also have to handle the (many) cases in which the vendor did not sell enough items to have a meaningful rating. In this case, it makes sense to use NA as an indicator that the rating is not present at all.
#TODO: clean rating keeping only number, set NA where not numeric or sufficient ratings earned.
...
```
Once we drawn a prleiminary profile of the market categories, we now focus on prices.
...
btc_eur_rate
```
Now we can use this rate to create a new column in our dataset.
```{r}
agora <-
  agora %>%
...
#TODO: treemap of category shares or vendors, rating clean and analysis on vendors
```{r}
categories <-
  agora %>%
  mutate(Depth = str_count(Category, "/") + 1) %>%
