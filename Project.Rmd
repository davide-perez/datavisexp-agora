...
agora %>%
  ...
  distinct()
```
We start by handling the missing ratings, which mean that the vendor did not close enough transactions: in this case, it makes sense to use NA as an indicator that the rating is not present at all.
```{r}
agora <-
  agora %>%
  mutate(Rating = ifelse(str_detect(Rating, "deals"), NA, Rating))
```
Then we clean the other values.
```{r}
agora <-
  agora %>%
...
```
We successfully cleaned the Rating feature.
```{r}
agora %>%
  select(Rating) %>%
  distinct() %>%
