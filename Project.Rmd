...
We then join the two cleaned datasets with the original dataset and replace the `Origin` and `Destination` column with their clean versions, and drop rows which could not be cleaned.
```{r}
agora <-
  inner_join(agora, origin_clean)
agora <-
  inner_join(agora, destination_clean)
...
#TODO: a few lines have the negation, consider clearing them. Pivot longer to get duplicate entries for items which ship to more than a country.
```{r}
agora %>%
  select(Destination) %>%
  distinct()
